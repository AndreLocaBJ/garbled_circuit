...
def expand_dont_care(truth_table, dv):
    ...
    while i < len_truth_table:
        if '-' in truth_table[i]['inp']:
            ...
            truth_table[i]['inp'] = [truth_table[i]['inp'][a:a+dv] for a in range(0, len(truth_table[i]['inp']), dv)]
    ...
    while i < len_truth_table:
        if '-' in truth_table[i]['out']:
            ...
            truth_table[i]['out'] = [truth_table[i]['out'][a:a+dv] for a in range(0, len(truth_table[i]['out']), dv)]
...
def crete_blif_mv(mv_table, mv, nomefile):
    import string
    # string.ascii_lowercase -> tutte le lettere da a a z in minuscolo
    mv_input = [i for i in list(string.ascii_lowercase)[:len(mv_table[0]['inp'])]]
    ...
    with open('./facili/blfmv/{}.blfmv'.format(nomefile), 'w') as blif:
        blif.write('.model {}\n'.format(nomefile))
...
def create_pla_expanded(inp,out,inp_array, out_array, truth_table, name_file):
    with open('./facili/ext_pla/{}_ext.pla'.format(name_file), 'w') as pla:
...
if __name__ == "__main__":
    try:
        ...
        for pla in glob('facili/starter/*.pla'):
            print(pla)
            inp, out, inp_array, out_array, truth_table = read_pla(pla)
