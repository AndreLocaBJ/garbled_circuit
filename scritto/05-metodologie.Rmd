...
with open('calcolo_costi.csv', 'a') as file:
  ...
  for blfmv in listdir('{}/blfmv'.format(working_dir)):
    if blfmv.endswith('.mv'):
      ...
      file.write('{};{};{};{};{};{};{};{}\n'.forma(
        blfmv.split('.')[0], costo_bool, alice_var_bool,bob_var_bool, 
          circ_mv['dominio'],costo_mv, alice_var_mv, bob_var_mv))
```
  ...
  for pla in listdir('{}/pla'.format(working_dir)):
    if pla.endswith('.pla'):
      ...
      if dv >= 8:
        ...
        mv = pow(2, dv)
        print('{}\ninp: {}\nout:{}\nmulti-valore:{}\n
          divide ogni:{}\n'.format(pla,inp.strip(), 
          out.strip(), int(mv), int(dv)))
